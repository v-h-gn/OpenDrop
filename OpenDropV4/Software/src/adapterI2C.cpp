...
float OpenAdapter::get_Temp_1(void)
{
  float temp = 0;
  if ((_ID & 2) == 2)
    temp = float(read8(ADAPTER_GETTEMP_L_1)) / 100 + read8(ADAPTER_GETTEMP_H_1);
  return temp;
};
...
float OpenAdapter::get_Temp_2(void)
{
  float temp = 0;
  if ((_ID & 2) == 2)
    temp = float(read8(ADAPTER_GETTEMP_L_2)) / 100 + read8(ADAPTER_GETTEMP_H_2);
  return temp;
};
...
float OpenAdapter::get_Temp_3(void)
{
  float temp = 0;
  if ((_ID & 2) == 2)
    temp = float(read8(ADAPTER_GETTEMP_L_3)) / 100 + read8(ADAPTER_GETTEMP_H_3);
  return temp;
};
...
{
  if ((_ID & 2) == 2)
    write8(ADAPTER_SETTEMP_H_1, temperature);
};

void OpenAdapter::set_Temp_2(uint8_t temperature)
{
  if ((_ID & 2) == 2)
    write8(ADAPTER_SETTEMP_H_2, temperature);
};

void OpenAdapter::set_Temp_3(uint8_t temperature)
{
  if ((_ID & 2) == 2)
    write8(ADAPTER_SETTEMP_H_3, temperature);
};
